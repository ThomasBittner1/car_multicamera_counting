...
class AppConfig:
    ...
    model_path: Path = PROJECT_ROOT / "models" / "cars_1.engine"
    window_names: tuple[str, ...] = ("source c042", "query c041")
    video_paths: tuple[str, ...] = (
        r"AICity22_Track1_MTMC_Tracking\test\S06\c042\vdo.avi",
        r"AICity22_Track1_MTMC_Tracking\test\S06\c041\vdo.avi",
    )
    entry_line_query: tuple[tuple[int, int], tuple[int, int]] = ((227, 283), (731, 956))

    source_discard_lines: tuple[tuple[tuple[int, int], tuple[int, int]], ...] = (
        ((411, 131), (5, 464)),
        ((298, 953), (963, 307)),
    )
    mask_points_by_camera: tuple[tuple[tuple[int, int], ...], ...] = (
        (
    ...
    not_from_other_camera_masks_query_camera: tuple[tuple[tuple[int, int], ...], ...] = (
        ((657, 948), (1083, 286), (1278, 419), (1277, 956)),
        ((2, 370), (536, 188), (888, 162), (1277, 199), (1275, 5), (2, 4)),
    )

    display_colors_by_camera: tuple[tuple[int, int, int], ...] = ((255, 0, 0), (0, 255, 0))
    display_inference_ignore_area_color: tuple[int, int, int] = (255, 0, 0)
    display_inference_ignore_area_alpha: float = 0.5
    display_not_from_other_camera_area_color: tuple[int, int, int] = (0, 0, 255)
